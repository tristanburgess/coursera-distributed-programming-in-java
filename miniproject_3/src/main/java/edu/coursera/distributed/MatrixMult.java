...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int localRowChunkSize = (nRows + size - 1) / size;
        final int localRowStart = rank * localRowChunkSize;
        final int localRowEnd = Math.min((rank + 1) * localRowChunkSize, nRows);
        ...
        // local mult computation
        for (int i = localRowStart; i < localRowEnd; i++) {
        ...
        if (rank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                final int reqRowStart = i * localRowChunkSize;
                final int reqRowEnd = Math.min((i + 1) * localRowChunkSize, nRows);
                final int reqRowOffset = reqRowStart * nCols;
                final int reqElemCount = (reqRowEnd - reqRowStart) * nCols;

                requests[i - 1] = mpi.MPI_Irecv(c.getValues(), reqRowOffset, reqElemCount,
            ...
            mpi.MPI_Send(c.getValues(), localRowStart * nCols,
                    (localRowEnd - localRowStart) * nCols, 0, rank, mpi.MPI_COMM_WORLD);
        }
    }
}
